...
# Step 2
# Remove the app/src/ prefix that appears in the "Location" column (in e.g. 'app/src/main/java/com/moduscreate/java/security/codeql/App.java'),
# because this prefix appears in CI, but not locally
sed 's/,app\/src\//,/g' "$1" > "$TMP_FILE"
mv "$TMP_FILE" "$1"
